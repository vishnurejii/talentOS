...
export default function JobDetail() {
  ...
  if (!job) return <div className="p-20 text-center text-slate-900">Job not found</div>

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-8 flex items-center justify-center">
      <div className="max-w-3xl w-full bg-white border border-slate-200 shadow-sm rounded-3xl p-10 relative overflow-hidden">
        <div className="absolute top-[-20%] right-[-10%] w-[30vw] h-[30vw] bg-blue-600/5 blur-[100px] rounded-full pointer-events-none" />
        ...
        <div className="flex gap-3 mb-8">
          <span className="px-3 py-1 bg-slate-100 text-slate-600 text-sm rounded-full border border-slate-200">{job.job_type}</span>
          <span className="px-3 py-1 bg-emerald-50 text-emerald-700 text-sm rounded-full border border-emerald-200">Open</span>
        </div>
        ...
        {user?.role === 'CANDIDATE' && !applied && (
          <form onSubmit={handleApply} className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-blue-500 hover:bg-blue-50 transition-all cursor-pointer relative">
              ...
              {file ? (
                ...
                <>
                  <p className="text-slate-700 font-medium mb-1">Click or drag CV here</p>
                  <p className="text-slate-500 text-sm">Required format: PDF</p>
                </>
              )}
            </div>
            <button disabled={isApplying} className="w-full mt-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:hover:scale-100 text-white font-medium py-3 rounded-xl transition-all shadow-lg shadow-blue-600/20 hover:scale-[1.02] active:scale-95">
        ...
        {user?.role === 'CANDIDATE' && applied && (
          <div className="bg-emerald-50 border border-emerald-200 p-8 rounded-2xl text-center shadow-sm">
            ...
            <p className="text-emerald-700 mb-6">Your CV is being parsed by our AI. You will be notified when it's time for the exam.</p>
            <Link to="/dashboard" className="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">Go to Dashboard</Link>
          </div>
        )}
      </div>
    </div>
